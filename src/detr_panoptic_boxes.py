...
def plot_cvt_results_to_file(frame, prob, boxes, panoptic_segm):
    yaml_writer = cv2.FileStorage("/tmp/detr_segm"+".yml", cv2.FileStorage_WRITE | cv2.FileStorage_FORMAT_YAML)
    yaml_writer.startWriteStruct("boxes", cv2.FileNode_SEQ)

    #box_list, conf_list, class_list = cvt_results(prob, boxes)
...
def plot_results_to_file(frame, prob, boxes, panoptic_segm):
    yaml_writer = cv2.FileStorage("/tmp/detr_segm"+".yml", cv2.FileStorage_WRITE | cv2.FileStorage_FORMAT_YAML)
    yaml_writer.startWriteStruct("boxes", cv2.FileNode_SEQ)
